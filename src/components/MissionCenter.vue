<template>
  <Transition name="mission-centre">
    <div
      v-if="isOpen && run"
      class="fixed inset-0 z-50 overflow-y-auto bg-black/55 p-4 text-white backdrop-blur-sm"
      @click.self="closeMissionCenter"
    >
      <section class="mx-auto flex max-h-[calc(100dvh-2rem)] w-[min(94vw,72rem)] flex-col overflow-hidden rounded-3xl border border-slate-700/80 bg-slate-900/94 shadow-2xl">
        <header class="sticky top-0 z-10 border-b border-slate-800 bg-slate-900/96 px-6 py-5 backdrop-blur-sm">
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              <p class="pixel-font text-[10px] uppercase tracking-[0.2em] text-amber-300/80">{{ run.modeLabel }}</p>
              <div class="mt-3 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.16em]">
                <span class="rounded-full border border-amber-400/20 bg-amber-400/10 px-3 py-1 text-amber-100">{{ run.story.actLabel }}</span>
                <span class="rounded-full border border-slate-700 bg-slate-950/70 px-3 py-1 text-slate-200">{{ run.story.chapterLabel }}</span>
              </div>
              <h2 class="mt-3 text-2xl font-semibold text-white">Mission {{ run.missionNumber }} · {{ run.story.title }}</h2>
              <p class="mt-2 max-w-2xl text-sm leading-relaxed text-slate-300">{{ run.story.kicker }}</p>
            </div>
            <button
          ...
          <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-4">
            <div class="rounded-2xl border border-slate-800 bg-slate-950/65 px-4 py-3">
              ...
              <p class="mt-2 font-mono text-2xl font-semibold text-white">{{ primaryGoalProgress }}</p>
            </div>
            <div class="rounded-2xl border border-slate-800 bg-slate-950/65 px-4 py-3">
              <p class="text-[10px] uppercase tracking-[0.14em] text-slate-400">Total Score</p>
              <p class="mt-2 text-2xl font-semibold text-white">{{ run.score }}</p>
            </div>
            <div class="rounded-2xl border border-slate-800 bg-slate-950/65 px-4 py-3">
              <p class="text-[10px] uppercase tracking-[0.14em] text-slate-400">Mission Score</p>
              <p class="mt-2 text-2xl font-semibold text-white">{{ run.missionScore }}</p>
            </div>
            <div class="rounded-2xl border border-slate-800 bg-slate-950/65 px-4 py-3">
